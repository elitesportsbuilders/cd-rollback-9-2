<template>
  <div class="relative h-full w-full flex-1 flex">
      ...
      <div class="relative flex-1 flex flex-col min-h-0">
          <!-- This ref is crucial for Leaflet to find the correct DOM element -->
          <div ref="mapContainerRef" class="flex-1 w-full h-full"></div>
          
          <!-- Residential Mode Scanning Animations -->
          <div v-if="mode === 'residential' && scanStatus === 'scanning'" class="absolute inset-0 radar-grid z-[440] pointer-events-none"></div>
          <div v-if="mode === 'residential' && scanStatus === 'scanning'" class="absolute inset-0 bg-black bg-opacity-25 z-[450] pointer-events-none"></div>
          <div v-if="mode === 'residential' && scanStatus === 'scanning'" class="radar-scanner pointer-events-none"></div>
      </div>
      
      <!-- Top-Left Controls (Dynamic based on mode) -->
      ...
      <!-- Bottom-Left Legend (Dynamic based on mode) -->
      ...
      <!-- Right Sidebar (Dynamic based on mode) -->
...
onMounted(() => {
  // We must wait for the DOM to be ready
  nextTick(() => {
      if (mapContainerRef.value && !map) {
          // FIX: Initialize map using the ref to the container element
...
watch(scanMethod, (newMethod) => {
  ...
  if (newMethod === 'draw' && props.mode === 'residential') {
      ...
      if(drawControl._toolbars.draw?._modes.polygon.handler) {
          drawControl._toolbars.draw._modes.polygon.handler.enable();
      }
