...
import { COURT_DATA, LEAD_DATA, RESIDENTIAL_PROSPECT_DATA } from '@/data';
...
watch(() => [props.reportTemplate, props.prospectId], () => {
    if (props.reportTemplate === 'Residential Prospect Report' && props.prospectId) {
        template.value = 'Residential Prospect Report';
        selectedProspectId.value = props.prospectId.toString();
        // Fallback if the prospectId isn't in the current dataset
        if (!RESIDENTIAL_PROSPECT_DATA.find(p => p.id.toString() === selectedProspectId.value)) {
            selectedProspectId.value = RESIDENTIAL_PROSPECT_DATA[0]?.id.toString();
        }
...
watch(template, (newTemplate) => {
  selectedId.value = newTemplate === 'Court Condition Report' ? '1' : '';
  selectedProspectId.value = newTemplate === 'Residential Prospect Report' ? RESIDENTIAL_PROSPECT_DATA[0]?.id.toString() : null;
  reportContent.value = null;
});

const handleGeneratePreview = () => {
  if (template.value === 'Court Condition Report') {
    const court = COURT_DATA.find(c => c.id.toString() === selectedId.value);
    if (court) {
      reportContent.value = { type: 'court', data: court, title: 'Court Condition Report' };
    }
  } else if (template.value === 'New Leads Summary') {
    reportContent.value = { type: 'leads', data: LEAD_DATA, title: 'New Leads Summary' };
  } else if (template.value === 'Residential Prospect Report') {
      const prospect = RESIDENTIAL_PROSPECT_DATA.find(p => p.id.toString() === selectedProspectId.value);
      if (prospect) {
                    ...
                    <div class="space-y-4">
                        <div v-if="template === 'Court Condition Report'">
                            ...
                            <select id="report-target" v-model="selectedId" class="mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                                <option v-for="court in COURT_DATA" :key="court.id" :value="court.id">{{ court.name }}</option>
                            </select>
                        </div>
                        <div v-else-if="template === 'New Leads Summary'">
                         ...
                         <div v-else-if="template === 'Residential Prospect Report'">
                            ...
                            <select id="report-target-residential" v-model="selectedProspectId" class="mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                                <option v-for="prospect in RESIDENTIAL_PROSPECT_DATA" :key="prospect.id" :value="prospect.id">{{ prospect.homeowner }} - {{ prospect.address }}</option>
                            </select>
                        </div>
                    </div>
                </div>
            ...
            <section class="flex-1 bg-slate-200 p-6">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-xl font-bold text-slate-700">Preview</h3>
                     <button class="px-4 py-2 bg-slate-700 text-white font-semibold rounded-lg shadow-sm hover:bg-slate-800 flex items-center text-sm disabled:bg-slate-400 disabled:cursor-not-allowed" :disabled="!reportContent">
